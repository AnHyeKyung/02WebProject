...
$('.main_collection .prd_tab #tab_01').addClass('on');

$('.main_collection .prd_tab .tab_bt ul li').click(function(){
    $('.main_collection .prd_tab .tab_bt ul li').removeClass('on');
    $(this).addClass('on');
})
function tab_open(x){
    for(var i = 1; i < 5; i++){
        $('.main_collection .prd_tab #tab_area_0'+i).css('display','none');
        //$('#tab_0'+i).attr('src','/SkinImg/img/0'+i+'.jpg');
    }
    $('.main_collection .prd_tab #tab_area_'+x).css('display','block');
...
$(document).ready(function(){
    ...
    if(document.getElementById('board_no')){
        var num = document.getElementById('board_no').value; 
        if(num == 1){
           $('#board01').css("color", "#999");
           $('#board01').css("text-decoration", "underline");
        } else if(num == 5){
            $('#board02').css("color", "#999");
            $('#board02').css("text-decoration", "underline");
        } else if(num == 4){
            $('#board03').css("color", "#999");
            $('#board03').css("text-decoration", "underline");
        } else if(num == 8){
            $('#board04').css("color", "#999");
            $('#board04').css("text-decoration", "underline");
        } else if(num == 3){
            $('#board05').css("color", "#999");
            $('#board05').css("text-decoration", "underline");
        } else if(num == 13){
            $('#board13').css("color", "#999");
            $('#board13').css("text-decoration", "underline");
        }  
        
    }
    
    if(document.getElementById('category_no')){
        var num = document.getElementById('category_no').value; 
        if(num == 1){
            document.getElementById('cate01').style.color="#999";
            document.getElementById('cate01').style.borderBottom="1px solid #999";
            document.getElementById('board05').style.display="block";
        } else if(num == 2){
            document.getElementById('cate02').style.color="#999";
            document.getElementById('cate02').style.borderBottom="1px solid #999";
            
        } else if(num == 970){
            document.getElementById('cate03').style.color="#999";
            document.getElementById('cate03').style.borderBottom="1px solid #999";
            
        }
        
    }
    	if(document.getElementById('cate')){
        var num = document.getElementById('cate').value; 
        if(num == 63){
            document.getElementById('t_nav01').style.color="#1e22aa";
            document.getElementById('cate01').style.fontWeight="500";
            document.getElementById('cate01').style.borderBottom="3px solid #1e22aa";
        } else if(num == 2){
            document.getElementById('cate02').style.color="#1e22aa";
            document.getElementById('cate02').style.fontWeight="500";
            document.getElementById('cate02').style.borderBottom="3px solid #1e22aa";
	...
	if ($('.youth_detail_info').length>0){
		$('.youth_detail_info').find('tr').each(function(){
			var th_text=$(this).find('th span').text();
			if (th_text!='상품명'&&th_text!='상품요약정보'&&th_text!='상품간략설명'&&th_text!='판매가'&&th_text!='할인판매가'){
    ...
    if($('.prdList').length>0){
        $('.prdList > li').each(function(){
            var li_this = $(this);
            if(li_this.find('.thumbnail > img').attr('alt')=='품절'){
                li_this.addClass('soldout_prd');
                li_this.find('.xans-product-listitem > li').each(function(){
                    if($(this).find('>strong>span').text()=='판매가') {
						$(this).addClass('soldout_price');
                    } else if($(this).find('>strong>span').text()=='할인판매가' || $(this).find('>strong>span').text()=='소비자가'){
                        $(this).addClass('displaynone');
                    }
                });
            }            
